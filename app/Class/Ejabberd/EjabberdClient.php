...
class EjabberdClient
{
    ...
    {
        $host = env('XMPP_API_URL', 'http://localhost:5280');
        $this->server = env('XMPP_SERVER_DOMAIN', 'localhost');

        $admin_auth_user = env('XMPP_ADMIN_USER', 'admin@localhost');
        $admin_auth_pass = env('XMPP_ADMIN_PASS', 'kkfvy');
    ...
    public function httpPost($uri, array $json = [])
    {
        try {
            ...
            return ['status'=>'error', 'ejabberd'=>$result];
        }
        $result = json_decode($response, true);
        # if array
     ...
     * @return mixed
     */
    public function httpGet($uri, array $data = [])
    {
        try {
    ...
    public function changeRoomOption(string $roomName, string $option = 'members_only', $value = 'true', $service = null)
    {
        return $this->httpPost('/api/change_room_option', [
            ...
            "option" => $option,
            "value" => $value,
            "service" => $service ?? 'conference.' . $this->server,
        ]);
    }
    ...
    {
        return $this->httpGet('/api/check_account', [
            "user" => $user,
            "host" => $host ?? $this->server
        ]);
    }
     ...
     * @return mixed Status code (0 on success, 1 otherwise)
     */
    public function createRoom(string $roomName, string $service = null)
    {
     ...
     * @return mixed Status code (0 on success, 1 otherwise)
     */
    public function createRoomWithOpts(string $roomName, array $options, string $service = null)
    {
    ...
    {
        return $this->httpGet('/api/get_roster', [
            "user" => $user,
            "host" => $server ?? $this->server
        ]);
    }
    ...
    public function getUserSubscriptions(string $user, $host = null)
    {
        return $this->httpPost('/api/get_user_subscriptions', [
            "user" => $user,
            "host" => $host ?? $this->server
        ]);
    }
    ...
    public function leaveCluster($node = null)
    {
        return $this->httpPost('/api/leave_cluster', [
            "node" => $node ?? "ejabberd@" . $this->server
        ]);
    }
     ...
     *  ]
     */
    public function mucOnlineRooms($service=null)
    {
    ...
    public function mucRegisterNick(string $jid, string $nick, $service = null)
    {
        return $this->httpPost('/api/muc_register_nick', [
            "jid" => $jid,
            "nick" => $nick,
            "serverhost" => $service ?? 'conference.' . $this->server,
        ]);
    }
    ...
    public function srgUserAdd(string $localUser, string $group, $host = null, string $grouphost = null)
    {
        return $this->httpPost('/api/srg_user_add', [
            ...
            "group" => $group,
            "host" => $host ?? $this->server,
            "grouphost" => $grouphost ?? $this->server,
        ]);
    }
    ...
    public function subscribeRoom(string $user, string $nick, string $roomName, $nodes = "urn:xmpp:mucsub:nodes:messages,urn:xmpp:mucsub:nodes:affiliations")
    {
        return $this->httpPost('/api/subscribe_room', [
            ...
            "nick" => $nick,
            "room" => strpos($roomName, 'conference') !== false ? $roomName : $roomName . '@conference.' . $this->server,
